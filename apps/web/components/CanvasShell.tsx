...
function defaultDataFor(kind: NodeKind): GenerativeNodeData {
  return {
    prompt: "",
    status: "idle",
  };
...
function CanvasInner({ projectId }: { projectId: string }) {
  ...
  const [, setStatus] = useState<"connecting" | "connected" | "disconnected">(
    "connecting",
  );
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        ...
        ids.forEach((id) => {
          ...
          const connected = wf.edges.filter(
            (ed) => ed.source === id || ed.target === id,
          );
          connected.forEach((ed) =>
            undoOps.push({ type: "op:edge:add", edge: ed }),
          );
          removeNode(id);
  ...
  useEffect(() => {
    ...
    const onRestore = (payload: { lineageId: string; workflow: Workflow }) => {
    ...
    socket.on("lineage:restore" as any, onRestore);
    ...
    return () => {
      ...
      socket.off("lineage:restore" as any, onRestore);
  ...
  const displayWorkflow = useMemo(() => {
    if (compareViewSide === "before" && compareBefore) return compareBefore;
    if (compareViewSide === "after" && compareAfter) return compareAfter;
    return workflow;
  }, [compareViewSide, compareBefore, compareAfter, workflow]);
  const isPreviewing = compareViewSide !== "diff" && compareDiff !== null;

  const rfNodes = useMemo(
    () => displayWorkflow.nodes.map(toRfNode),
    [displayWorkflow.nodes],
  );
  const rfEdges = useMemo(
    () => displayWorkflow.edges.map(toRfEdge),
    ...
    (changes: NodeChange[]) => {
      ...
      changes.forEach((ch) => {
        if (ch.type === "position" && ch.position) {
          ...
          const undoOps: Operation[] = [];
          if (node) undoOps.push({ type: "op:node:add", node });
          const connected = workflow.edges.filter(
            (e) => e.source === ch.id || e.target === ch.id,
          );
          connected.forEach((e) =>
            undoOps.push({ type: "op:edge:add", edge: e }),
          );
          if (undoOps.length > 0) pushUndo({ ops: undoOps });
          removeNode(ch.id);
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const rfPane = wrapper.querySelector(
  ...
  const canvasBg = isDark ? "#141414" : "#FAFAFA";
    ...
    >
      <div
        style={{
          width: "100%",
          height: "100%",
          opacity: compareDiff && !isPreviewing ? 0.55 : 1,
      ...
      >
        <ReactFlow
          ...
          edges={rfEdges}
          nodeTypes={nodeTypes}
          onNodesChange={isPreviewing ? undefined : onNodesChange}
          onEdgesChange={isPreviewing ? undefined : onEdgesChange}
          onConnect={isPreviewing ? undefined : onConnect}
          onSelectionChange={isPreviewing ? undefined : onSelectionChange}
          nodesDraggable={!isPreviewing}
          nodesConnectable={!isPreviewing}
          elementsSelectable={!isPreviewing}
          panOnDrag={false}
          panOnScroll
          selectionOnDrag={!isPreviewing}
          proOptions={{ hideAttribution: true }}
          fitView
          style={{ background: canvasBg, cursor: "default" }}
          panOnScrollSpeed={1}
          maxZoom={2}
          minZoom={0.1}
        >
...
export function CanvasShell({ projectId }: { projectId: string }) {
  ...
  if (!mounted) {
    ...
    return (
      <div style={{ height: "100vh", width: "100vw", background: "#fafbff" }} />
    );
  }
  return (
    <ReactFlowProvider>
